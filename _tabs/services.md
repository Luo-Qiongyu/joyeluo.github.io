---
# the default layout is 'page'
title: Services
icon: fas fa-car
order: 4
---

<h1 style="display: flex; align-items: center; gap: 0.35em;">
  <img src="/assets/img/contact/jaf-logo.svg" alt="JAF 官方标识" width="96">
  <span style="position: relative; top: -0.12em;">JAF 驾照翻译件代办</span>
</h1>

提供 JAF 驾照翻译件代办服务。可协助办理日本租车、日本驾照在台湾租车、外国驾照在日本进行外免切替、中国驾照换日本驾照等场景中可能需要的 JAF 翻译件。

需要办理可直接滑到页面底部扫码咨询，确认驾照类型和打印便利店后再下单。

翻译件出来之后会给您发送打印码，并根据您选择的便利店提供对应打印步骤。打印码默认用于日本 711 便利店打印。如果需要在 Lawson、FamilyMart、Ministop、Poplar Group 打印，可以联系店主发行新的打印码。

<small>※ 只有在日本便利店使用打印码打印，才会打印出带有 JAF 专用防伪水印的翻译件。</small>

<small>※ 日本大部分机场 711 便利店没有打印机，如果需要在机场打印，建议提前联系我发行 Lawson 或 FamilyMart 便利店可用的打印码。</small>

## 翻译件样张

<div style="display: flex; gap: 1rem; align-items: flex-start; flex-wrap: wrap;">
  <img src="/assets/img/contact/jaf-translation-sample.jpg" alt="JAF 翻译件样张" width="340" loading="eager">
  <img src="/assets/img/contact/jaf-appendix-sample.jpg" alt="JAF 翻译件附页样张" width="340" loading="eager">
</div>

<small>※ 样张图片来自 JAF 英文官网，仅用于展示翻译件格式。</small>

## 适用范围
<strong>1.</strong> 持短期日本签证，计划在日本自驾或租车的客人，如果持有以下国家或地区的驾照，可申请 JAF 日文翻译件后在日本驾驶：
<div style="display: flex; gap: 0.75rem; align-items: flex-start; flex-wrap: wrap;" markdown="1">
<div style="flex: 0 0 180px;" markdown="1">
- 瑞士
- 德国
- 法国
- 比利时
- 摩纳哥
- 台湾
</div>
<div style="flex: 0 1 300px; margin-top: -0.35rem;">

<img src="/assets/img/contact/jaf-official-sample-japan-small.png" alt="外国驾照和 JAF 日文翻译件可在日本驾驶的官方示意图" width="300" loading="eager">

</div>
</div>

其他国家或地区的驾照也可以协助代办 JAF 翻译件，但通常不能仅凭该翻译件在日本租车或自驾。此类翻译件更常用于外免切替、资料提交或相关手续咨询。

注意：中国大陆驾照 + JAF 翻译件不能在日本租车和自驾，只能用于中国驾照换日本驾照考试等相关手续。

<strong>2.</strong> 日本驾照办理台湾 JAF 翻译件后，可以在台湾租车和自驾。

<div style="text-align: left;">

<img src="/assets/img/contact/jaf-official-sample-taiwan-small.png" alt="日本驾照和 JAF 中文翻译件可在台湾驾驶的官方示意图" width="300" loading="eager">

</div>

## JAF 翻译件代办须知

1. 代办服务只是协助不在日本的客人申请 JAF 翻译件，翻译件是否可以成功办理取决于 JAF 官方审核。如申请不成功，可全额退款。

2. 翻译件一般 1-2 个工作日出具，最长可能需要两周以内。请根据出行时间提前申请。

3. 客人在租车行、驾驶途中或使用翻译件过程中发生的任何问题，均与代办无关，代办不承担任何责任。请直接联系租车公司、警察、保险公司、JAF 或其他相关机构处理。

4. 申请成功后会发行一个打印码。凭打印码可在日本任意 711 便利店打印，也可根据需要发行 Lawson、FamilyMart、Ministop、Poplar Group 可用的打印码。

5. 打印码有效期为 7 天。7 天后如果没有打印，可以联系我重新发行打印码。一年内都可以重新发行，重新发行打印码通常几分钟即可完成，不需要再次付费。

6. JAF 翻译件有效期与您的驾照有效期一致。

7. 关西机场和新千岁机场内的 Lawson 便利店有打印机可以打印，冲绳机场内的 FamilyMart 便利店有打印机可以打印。如果需要在这些机场打印，请提前告知，我会给您发行 Lawson 和 FamilyMart 打印码。

## 准备材料

- 驾照正反面清晰照片，手机拍摄时请露出驾照四个角。
- 希望使用的打印便利店：711、Lawson、FamilyMart、Ministop 或 Poplar Group。

## 服务价格

| 服务类型 | 价格 |
| --- | --- |
| 外国驾照 → 日本 JAF 翻译件 | 360 人民币 |
| 日本驾照 → 台湾 JAF 翻译件 | 420 人民币 |

以上价格均包含 JAF 申请在内的所有费用。

## 官方参考

关于外国驾照在日本驾驶、外国驾照换日本驾照等制度说明，可参考日本警察厅官网：

[外国の運転免許をお持ちの方](https://www.npa.go.jp/policies/application/license_renewal/have_DL_issed_another_country.html)

关于 JAF 翻译件样张和适用场景，可参考 JAF 英文官网：

[About DLTAS](https://english.jaf.or.jp/driving-in-japan/drive-in-japan/about-dltas)

## 联系方式

欢迎通过邮件联系我，并附上你的驾照签发国家或地区、预计出行时间和打印便利店需求。

- Email: <qiongyu.luo@gmail.com>

### 微信咨询

<div style="text-align: left;">

<img src="/assets/img/contact/wechat-qr.jpg" alt="微信咨询二维码" width="260">

</div>

### 淘宝下单

<div style="text-align: left;">

<img src="/assets/img/contact/taobao-qr.jpg" alt="淘宝店铺二维码" width="320">

</div>

也可以点击淘宝链接打开：[JAF驾照翻译/JAF翻译件/海外租车/日本租车/中国换日本驾照翻译件](https://e.tb.cn/h.RPDPGmheRJRH4zG?tk=SmSJg2AYiZl)
